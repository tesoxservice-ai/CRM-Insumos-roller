...
// Modal para crear un nuevo cliente en el CRM.
// Usa estado local de React (sin react-hook-form).
// Cierra con X, Escape o click en el overlay.
...
const inputClass =
  'w-full rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-900 outline-none placeholder:text-gray-400 focus:border-gray-400 focus:ring-2 focus:ring-gray-100 transition-colors'
...
export default function NuevoClienteModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Validar teléfono si fue completado
    if (form.telefono.trim()) {
      const err = validarTelefono(form.telefono)
      if (err) {
        setTelefonoError(err)
        return
      }
    }
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="flex flex-col gap-4 px-6 py-5">
            ...
            >
              <input
                ...
                className={[
                  inputClass,
                  telefonoError ? 'border-red-300 focus:border-red-400 focus:ring-red-100' : '',
                ].join(' ')}
              />
              {telefonoError && (
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Canal de entrada">
                <select
                  ...
                  onChange={(e) => setField('canal_entrada', e.target.value as CanalEntrada)}
                  className={inputClass}
                >
          ...
          <div className="flex justify-end gap-2 border-t border-gray-100 px-6 py-4">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 disabled:cursor-not-allowed disabled:opacity-50 transition-colors"
            >
              {loading ? (
                <>
